...
> [!NOTE]
> Security advisories that pertain to artifacts provided by Julia packages (notably JLLs) are published on a best-effort basis and known to be incomplete.
...
All advisories in the `advisories/published` folder are automatically exported to JSON files grouped by year in [valid OSV schema](https://ossf.github.io/osv-schema/) on the [`generated/osv` branch](https://github.com/JuliaLang/SecurityAdvisories.jl/tree/generated/osv). Pull request CI does a dry-run of this conversion and validation before merging.

## The tooling

Most of the code in this repository is built to search, convert, and draft advisories that have been published in other databases and pertain either to a Julia package directly (an alias) or pertain to some artifact it redistributes (an upstream).
...
* GitHub's GHSA: GitHub Security Advisories have their own identifiers and may or may not have a CVE alias. Some GHSAs are "originated" by GitHub, but most are imported from NVD. Note that there are three flavors of GHSAs:
...
* [JuliaRegistries/General](https://github.com/JuliaRegistries/General): The default Pkg registry provides the available package names along with their available versions and upstream repository.
...
There are hundreds of thousands of GitHub advisories and CVEs. The GitHub actions on this repository aim to find new vulnerabilities that have been registered, affect Julia packages, and automatically open pull requests suggesting their inclusion in this database. The biggest challenge is in accurately identifying the relevant package(s) and version(s).
...
* **Advisories published on a registered package's GitHub repository**. A GitHub-based package maintainer can create a new GHSA security advisory directly against the canonical (registered) package repository. The versions listed within this GHSA directly correspond to the registered and vulnerable versions, and the JLSEC here **must** have this GHSA listed as one of its `alias`es.
...
and will open pull requests suggesting the inclusion of these new advisories. Twice a day, a scheduled search
covers every package with a version registered since the last run and every package affected by an upstream
